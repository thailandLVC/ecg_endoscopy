################################################################################
## Decision tree model for full probabilistic sensitivity analysis
################################################################################
...
cascadeE <- read.csv("~/cascade_seq_ekg.csv")
...
ekg <- rgamma(n = 1000, shape = costs$alpha[1], scale = costs$beta[1])
...
for (i in 1:1000){
  ...
  attach_costs <- function(node){
    if (node$name == "EKG"){node$cost <-  ekg[i]}
